"""AtlasFind tool dataset validation helpers for v0.6.1."""
...
ALLOWED_QUALITY_STATUSES = {"verified", "partially_verified", "unverified", "review_due"}
...
def validate_tool(tool: Any, index: int) -> list[str]:
    ...
    quality_status = str(tool.get("quality_status", "")).strip().lower()
    if quality_status and quality_status not in ALLOWED_QUALITY_STATUSES:
        errors.append(f"{label}.quality_status: must be one of {sorted(ALLOWED_QUALITY_STATUSES)}")

    quality_review = tool.get("quality_review")
    if isinstance(quality_review, dict):
        for field in ("scope", "reviewed_at", "note"):
            if not _is_non_empty_text(quality_review.get(field)):
                errors.append(f"{label}.quality_review.{field}: required non-empty text")
        if _is_non_empty_text(quality_review.get("reviewed_at")) and not _valid_iso_date(quality_review.get("reviewed_at")):
            errors.append(f"{label}.quality_review.reviewed_at: must use a valid YYYY-MM-DD date")
    ...
    for field in ("icon_url", "icon_source", "icon_alt", "icon_fallback_url"):
        if field in tool and not _is_non_empty_text(tool[field]):
            errors.append(f"{label}.{field}: value cannot be empty")

    icon_meta = tool.get("icon_meta")
    if isinstance(icon_meta, dict):
        if icon_meta.get("fallback") != "local-svg-monogram":
            errors.append(f"{label}.icon_meta.fallback: must use local-svg-monogram")
        if not _is_non_empty_text(icon_meta.get("initials")):
            errors.append(f"{label}.icon_meta.initials: required non-empty text")
