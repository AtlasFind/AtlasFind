...
def load_queue() -> dict:
    ...
    return {"version": 1, "updated_at": now(), "status": "research_only", "items": []}
...
def discover_once(queue: dict, *, per_query: int, min_stars: int, max_candidates: int) -> int:
    ...
    for query, category, subcategory in SEARCHES:
        if len(queue["items"]) >= max_candidates:
            break
        try:
            result = github_get(
                "/search/repositories",
...
def main() -> None:
    ...
    if not 1 <= args.max_candidates <= 2000:
        raise SystemExit("--max-candidates must be between 1 and 2000")
    queue = load_queue()
    deadline = time.monotonic() + max(0.05, args.hours) * 3600
    log(f"Worker started: target={args.max_candidates}, hours={args.hours}, existing={len(queue['items'])}")
    while time.monotonic() < deadline and len(queue["items"]) < args.max_candidates:
        ...
        if args.once or len(queue["items"]) >= args.max_candidates:
            break
