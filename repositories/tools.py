...
def get_all_tools(path=DATABASE_PATH):
    ...
    with connect_database(path) as connection:
        rows = connection.execute("SELECT payload_json FROM tools WHERE COALESCE(status,'published')='published' ORDER BY id").fetchall()
...
def get_tool_by_slug(slug, path=DATABASE_PATH):
    if not path.exists():
        return None
    with connect_database(path) as connection:
        row = connection.execute("SELECT payload_json FROM tools WHERE slug = ? AND COALESCE(status,'published')='published'", (slug,)).fetchone()
