...
class ManualExpansionTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.tools = json.loads((ROOT / "data/tools.json").read_text(encoding="utf-8"))
        cls.latest_additions = cls.tools[850:]

    def test_catalog_is_exactly_1000_unique_schema_valid_tools(self):
        self.assertEqual(1000, len(self.tools))
        self.assertEqual(1000, len({tool["id"] for tool in self.tools}))
        self.assertEqual(1000, len({tool["slug"].casefold() for tool in self.tools}))
        self.assertEqual([], validate_tools(self.tools))

    def test_latest_150_additions_are_published_and_logo_verified(self):
        self.assertEqual(list(range(851, 1001)), [tool["id"] for tool in self.latest_additions])
        for tool in self.latest_additions:
    ...
    def test_release_audits_are_complete(self):
        sources = json.loads((ROOT / "reports/manual-expansion-1000-source-audit.json").read_text(encoding="utf-8"))
        logos = json.loads((ROOT / "reports/manual-expansion-1000-logo-import.json").read_text(encoding="utf-8"))
